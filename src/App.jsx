...
// Sirf zaroori icons import karein
import { 
  AlertCircle,
  Printer,
  CheckSquare,
...
// Import utilities
import { formatCurrency, formatDate, generateOrderId } from './utils/formatters';
...
export default function FleetXApp() {
  ...
  useEffect(() => {
    if (editingTrip) {
      ...
      if (liveTrip) {
        if(JSON.stringify(liveTrip) !== JSON.stringify(editingTrip)) {
    ...
    <div className={`bg-slate-900 text-white h-full flex flex-col ${mobile ? 'w-full' : 'w-64'} print:hidden`}>
      <div className="p-6 border-b border-slate-800">
        <div className="flex items-center gap-3 mb-1">
          <div className="w-8 h-8 bg-blue-600 rounded-lg flex items-center justify-center"><Truck size={18}/></div>
      ...
      <nav className="flex-1 p-4 space-y-1">
        {[
          ...
          <button 
            key={item.id} 
            onClick={() => { setActiveView(item.id); if(mobile) setIsMobileMenuOpen(false); setDashboardFilter(null); }} 
            className={`w-full flex items-center gap-3 p-3 rounded-xl transition-all ${activeView === item.id ? 'bg-blue-600 shadow-lg shadow-blue-900/50' : 'text-slate-400 hover:bg-slate-800 hover:text-white'}`}
          >
            <item.icon size={18} /> 
