...
export default function FleetXApp() {
    ...
    for (let req of required) {
      ...
      if (!exists && user) {
         try {
           ...
           console.log(`Created system account: ${req.name}`);
         } catch (e) {
  ...
  // Run check when accounts update
  useEffect(() => {
  ...
  const TripManager = () => {
    ...
    const handleTripSubmit = async (e) => {
      ...
      try {
        ...
        if(!vehIncomeId || !tripExpId || !cashId) {
          return alert("System Accounts (Vehicle Income, Trip Expense, Cash) missing. Please refresh page.");
        }
        ...
        const entries = [{ accountId: tripData.clientId, type: 'debit', amount: freight }, { accountId: vehIncomeId, type: 'credit', amount: freight }];
        if (totalExpense > 0) { 
          entries.push({ accountId: tripExpId, type: 'debit', amount: totalExpense }); 
          entries.push({ accountId: cashId, type: 'credit', amount: totalExpense }); 
        }
    ...
    const handleDetentionSubmit = async (e) => {
      ...
      if(!incomeId) return alert("System Account 'Income' missing.");
      ...
      newEntries.push({ accountId: incomeId, type: 'credit', amount: totalAmount }); 
  ...
  const BillingView = () => { /* Same Billing */
