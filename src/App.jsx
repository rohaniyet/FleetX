...
export default function FleetXApp() {
  ...
  const getAccountId = (nameLike) => {
    const found = accounts.find(a => a.name.includes(nameLike));
    return found ? found.id : (accounts.find(a => a.category === 'Cash' || a.category === 'Cash/Bank')?.id || '');
  };
  ...
  const TripManager = () => {
    ...
    useEffect(() => {
      if(selectedOrderId) {
        ...
        if(ord) {
          ...
          setTripData(prev => ({
            ...prev, clientId: ord.clientId, route: `${ord.locationFrom} to ${ord.locationTo}`, croNo: ord.croNo, vehicleType: defaultType, weight: defaultType === 'Single' ? ord.weightSingle : ord.weightDouble, freightAmount: defaultType === 'Single' ? ord.rateSingle : ord.rateDouble
          }));
        }
      }
    ...
    const removeExpenseRow = (index) => setExpenses(expenses.filter((_, i) => i !== index));
    ...
    const handleTripSubmit = async (e) => {
      ...
      let totalExpense = 0;
      expenses.forEach(exp => { if(exp.amount) totalExpense += Number(exp.amount); });
      ...
      const entries = [{ accountId: tripData.clientId, type: 'debit', amount: freight }, { accountId: getAccountId('Vehicle Income'), type: 'credit', amount: freight }];
      if (totalExpense > 0) { entries.push({ accountId: getAccountId('Trip Expense'), type: 'debit', amount: totalExpense }); entries.push({ accountId: getAccountId('Cash'), type: 'credit', amount: totalExpense }); }
      
      await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'accounts', tripData.vehicleId), { currentLocation: 'On Trip' });

      await addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'transactions'), { ...tripData, orderId: selectedOrderId, description: desc, totalAmount: freight, expenseDetails: expenses.filter(e => e.name && e.amount), entries, createdAt: serverTimestamp(), type: 'Trip' });
      const ord = orders.find(o => o.id === selectedOrderId);
    ...
    const handleCloseTrip = async (trip) => {
       ...
       await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'transactions', trip.id), { status: 'Closed' });
       await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'accounts', trip.vehicleId), { currentLocation: destination.trim() });
       alert(`Trip Completed! Vehicle is now at ${destination}`);
       setEditingTrip(null);
    };

    const handleDetentionSubmit = async (e) => {
      e.preventDefault();
      if(!detentionData.tripId || (!detentionData.amount && !detentionData.loloAmount)) return;
      const detAmount = Number(detentionData.amount) || 0;
      const loloAmount = Number(detentionData.loloAmount) || 0;
      const totalAmount = detAmount + loloAmount;
      ...
      newEntries.push({ accountId: clientId, type: 'debit', amount: totalAmount });
      newEntries.push({ accountId: getAccountId('Income'), type: 'credit', amount: totalAmount }); 
      await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'transactions', detentionData.tripId), { detentionAmount: (trip.detentionAmount || 0) + detAmount, loloAmount: (trip.loloAmount || 0) + loloAmount, entries: newEntries, detentionRemarks: detentionData.remarks });
      alert("Charges Added!");
    ...
    if (editingTrip) {
      return (
        <div className="max-w-3xl mx-auto bg-white rounded-2xl border border-slate-200 shadow-lg animate-fade-in"><div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50 rounded-t-2xl"><div><h2 className="text-xl font-bold text-slate-800">Manage Trip</h2><p className="text-sm text-slate-500">{editingTrip.description}</p></div><button onClick={() => setEditingTrip(null)} className="p-2 hover:bg-slate-200 rounded-full"><X size={20}/></button></div><div className="p-6 space-y-6">
  ...
  const PaymentsManager = () => {
    ...
    const [fromRows, setFromRows] = useState([{ accountId: '', amount: '' }]);
    const [toRows, setToRows] = useState([{ accountId: '', amount: '' }]);

    const addRow = (type) => {
      if(type === 'from') setFromRows([...fromRows, { accountId: '', amount: '' }]);
      else setToRows([...toRows, { accountId: '', amount: '' }]);
    };

    const updateRow = (type, idx, field, val) => {
      const rows = type === 'from' ? [...fromRows] : [...toRows];
      rows[idx][field] = val;
      if(type === 'from') setFromRows(rows); else setToRows(rows);
    };

    const removeRow = (type, idx) => {
      if(type === 'from') setFromRows(fromRows.filter((_, i) => i !== idx));
      else setToRows(toRows.filter((_, i) => i !== idx));
    };

    const totalFrom = fromRows.reduce((sum, r) => sum + Number(r.amount), 0);
    const totalTo = toRows.reduce((sum, r) => sum + Number(r.amount), 0);

    const handleSubmit = async (e) => {
      e.preventDefault();
      if(totalFrom !== totalTo || totalFrom === 0) return alert("Total From and To amounts must match and be greater than 0");
      
      const entries = [];
      // From side (Credit)
      fromRows.forEach(r => {
        if(r.accountId && r.amount) entries.push({ accountId: r.accountId, type: 'credit', amount: Number(r.amount) });
      });
      // To side (Debit)
      toRows.forEach(r => {
        if(r.accountId && r.amount) entries.push({ accountId: r.accountId, type: 'debit', amount: Number(r.amount) });
      });

      await addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'transactions'), {
        description: description || 'Payment / Transfer',
        totalAmount: totalFrom,
        entries,
      ...
      alert("Compound Payment Recorded!");
      setDescription('');
      setFromRows([{ accountId: '', amount: '' }]);
      setToRows([{ accountId: '', amount: '' }]);
    };

    return (
      <div className="max-w-4xl mx-auto bg-white p-8 rounded-2xl border border-slate-200 shadow-sm animate-fade-in">
        <h3 className="font-bold text-xl mb-6 text-slate-800 flex items-center gap-2"><Banknote className="text-blue-600"/> Compound Payment Entry</h3>
        
        <div className="grid grid-cols-2 gap-6 mb-6">
           <div><label className="block text-xs font-bold text-slate-500 mb-1">Date</label><input type="date" className="w-full p-3 border border-slate-200 rounded-xl" value={date} onChange={e => setDate(e.target.value)} /></div>
           <div><label className="block text-xs font-bold text-slate-500 mb-1">Description</label><input type="text" className="w-full p-3 border border-slate-200 rounded-xl" placeholder="Remarks" value={description} onChange={e => setDescription(e.target.value)} /></div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
           ...
           <div className="bg-emerald-50 p-4 rounded-xl border border-emerald-100">
              <div className="flex justify-between items-center mb-3">
                 <h4 className="font-bold text-emerald-800">Amount From (Source/Credit)</h4>
                 <button onClick={() => addRow('from')} className="text-xs bg-white px-2 py-1 rounded shadow-sm">+ Add</button>
              </div>
              {fromRows.map((row, idx) => (
                <div key={idx} className="flex gap-2 mb-2">
                   <select className="flex-1 p-2 text-sm border rounded" value={row.accountId} onChange={e => updateRow('from', idx, 'accountId', e.target.value)}>
                      <option value="">Select Account</option>
                      {accounts.map(a => <option key={a.id} value={a.id}>{a.name}</option>)}
                   </select>
                   <input type="number" placeholder="Amt" className="w-20 p-2 text-sm border rounded" value={row.amount} onChange={e => updateRow('from', idx, 'amount', e.target.value)} />
                   <button onClick={() => removeRow('from', idx)} className="text-red-400"><X size={16}/></button>
                </div>
              ))}
              <div className="text-right font-bold text-emerald-700 mt-2">Total: {formatCurrency(totalFrom)}</div>
           </div>

           {/* TO SECTION (DEBIT) */}
           <div className="bg-blue-50 p-4 rounded-xl border border-blue-100">
              <div className="flex justify-between items-center mb-3">
                 <h4 className="font-bold text-blue-800">Amount To (Destination/Debit)</h4>
                 <button onClick={() => addRow('to')} className="text-xs bg-white px-2 py-1 rounded shadow-sm">+ Add</button>
              </div>
              {toRows.map((row, idx) => (
                <div key={idx} className="flex gap-2 mb-2">
                   <select className="flex-1 p-2 text-sm border rounded" value={row.accountId} onChange={e => updateRow('to', idx, 'accountId', e.target.value)}>
                      <option value="">Select Account</option>
                      {accounts.map(a => <option key={a.id} value={a.id}>{a.name}</option>)}
                   </select>
                   <input type="number" placeholder="Amt" className="w-20 p-2 text-sm border rounded" value={row.amount} onChange={e => updateRow('to', idx, 'amount', e.target.value)} />
                   <button onClick={() => removeRow('to', idx)} className="text-red-400"><X size={16}/></button>
                </div>
              ))}
              <div className="text-right font-bold text-blue-700 mt-2">Total: {formatCurrency(totalTo)}</div>
           </div>
        </div>

        <button 
          onClick={handleSubmit} 
          disabled={totalFrom !== totalTo || totalFrom === 0}
          className="w-full mt-6 bg-slate-900 text-white py-4 rounded-xl font-bold shadow-lg hover:bg-slate-800 disabled:opacity-50 disabled:cursor-not-allowed"
        >
          {totalFrom === totalTo ? 'Post Transaction' : 'Amounts Must Match'}
        </button>
      </div>
    );
  };
  ...
  const StoreManager = () => { /* Same Store Manager */
    const [newItem, setNewItem] = useState({ name: '', qty: '', cost: '' });
    const handleAddItem = async (e) => { e.preventDefault(); if (!newItem.name) return; await addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'inventory'), { name: newItem.name, qty: Number(newItem.qty), avgCost: Number(newItem.cost), value: Number(newItem.qty) * Number(newItem.cost), createdAt: serverTimestamp() }); setNewItem({ name: '', qty: '', cost: '' }); };
    return (
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-fade-in"><div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm h-fit"><h3 className="font-bold text-lg mb-4">Add Inventory</h3><form onSubmit={handleAddItem} className="space-y-4"><input type="text" placeholder="Item Name" className="w-full p-3 border border-slate-200 rounded-xl" value={newItem.name} onChange={e => setNewItem({...newItem, name: e.target.value})} /><div className="flex gap-3"><input type="number" placeholder="Qty" className="flex-1 p-3 border border-slate-200 rounded-xl" value={newItem.qty} onChange={e => setNewItem({...newItem, qty: e.target.value})} /><input type="number" placeholder="Cost" className="flex-1 p-3 border border-slate-200 rounded-xl" value={newItem.cost} onChange={e => setNewItem({...newItem, cost: e.target.value})} /></div><button type="submit" className="w-full bg-emerald-600 text-white py-3 rounded-xl font-medium">Add to Stock</button></form></div><div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm"><h3 className="font-bold text-lg mb-4">Current Stock</h3><div className="space-y-2 max-h-[400px] overflow-y-auto">{inventory.map(item => (<div key={item.id} className="p-3 bg-slate-50 rounded-lg flex justify-between items-center"><span className="font-medium text-slate-700">{item.name}</span><span className="text-sm font-bold bg-white px-2 py-1 rounded border border-slate-200">{item.qty} units</span></div>))}</div></div></div>
    );
  };
